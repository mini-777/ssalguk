...
function createExtractor({ logger = console } = { logger: console }) {
  ...
  function parseWithConfig({ components, tamaguiConfig }, fileOrPath, options) {
    ...
    function isValidStyleKey(name, staticConfig) {
      ...
      return platform === "native" && name[0] === "$" && mediaQueryConfig[name.slice(1)] ? !1 : !!((_a2 = staticConfig.validStyles) != null && _a2[name] || pseudoDescriptors[name] || // dont disable variants or else you lose many things flattening
    ...
    const isTargetingHTML = platform === "web", ogDebug = shouldPrintDebug, tm = (0, import_timer.timer)(), propsWithFileInfo = {
      ...options,
      sourcePath,
          ...
          if (shouldFlatten && shouldWrapTheme && (programPath ? (shouldPrintDebug && logger.info(["  - wrapping theme", themeVal].join(" ")), attrs = attrs.filter(
            ...
            const defaultStyleAttrs = Object.keys(defaultProps).flatMap((key) => {
              if (!isValidStyleKey(key, staticConfig))
                return [];
              const value = defaultProps[key], name = (tamaguiConfig == null ? void 0 : tamaguiConfig.shorthands[key]) || key;
              if (value === void 0) {
                logger.warn(
