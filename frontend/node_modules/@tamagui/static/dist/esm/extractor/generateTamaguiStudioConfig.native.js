...
async function generateTamaguiThemes(tamaguiOptions, force = !1) {
  if (!tamaguiOptions.themeBuilder)
    return;
  const { input, output } = tamaguiOptions.themeBuilder, inPath = resolveRelativePath(input), outPath = resolveRelativePath(output), generatedOutput = await generateThemes(inPath), hasChanged = force || await (async () => {
    try {
